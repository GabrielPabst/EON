...
class ActionsLogManager:
    ...
    def modify_event(self, index: int, **kwargs) -> None:
        ...
        # Check if key is being modified
        key_changed = 'key' in kwargs and kwargs['key'] != event.key
        
        # Store the old key type before any modifications
        old_is_mouse = self.is_mouse_key(event.key)
        new_is_mouse = self.is_mouse_key(kwargs.get('key', event.key))
        
        # Store original coordinates/screenshot to preserve them if needed
        original_x = event.x
        original_y = event.y
        original_screenshot = event.screenshot
        print(original_x, original_y, original_screenshot)
        
        # Store original linked event for rollback
        original_linked = None
        linked_index = None
        
        # If key is changing, update linked event's key as well
        if key_changed:
            linked_index = self._find_linked_event(index)
            if linked_index is not None:
                linked_event = self.events[linked_index]
                original_linked = copy.deepcopy(linked_event)
                
                # Update linked event's key
                linked_event.key = kwargs['key']
                
                # Only clear coordinates and screenshot if switching to non-mouse key
                if old_is_mouse and not new_is_mouse:
                    linked_event.x = None
                    linked_event.y = None
                    linked_event.screenshot = None
        
        # Apply modifications to the main event
        ...
        # Now handle coordinates and screenshots based on key type changes
        if key_changed:
            if old_is_mouse and not new_is_mouse:
                print("CLEAR")
                # Switching from mouse to non-mouse: clear everything
                event.x = None
                event.y = None
                event.screenshot = None
            elif old_is_mouse and new_is_mouse:
                event.x = original_x
                event.y = original_y
                event.screenshot = original_screenshot
        ...
        try:
            ...
            # Validate linked event coordinates if it was modified
            if key_changed and linked_index is not None:
                linked_event = self.events[linked_index]
                self.validate_coordinates(linked_event.key, linked_event.x, linked_event.y)
            ...
            if key_changed and linked_index is not None:
                self.events[linked_index] = original_linked
            raise
